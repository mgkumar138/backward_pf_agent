...
for gamma in gammas:
    for N in Ns:
        for sigma_init in sigma_inits:
            try:
                data = np.load(
                    f'./data/dcells_{sigma_init}sigma_{w_init}w_{eta0}eta0_{N}N_{gamma}gamma_{nstates}states_{seed}seed.npz',
                    allow_pickle=True
                )
                ...
                data_points.append([gamma, N, sigma_init, reward_cells, approach_cells, screen_cells])
            except FileNotFoundError:
                print(f"File not found for gamma={gamma}, N={N}, sigma_init={sigma_init}. Skipping...")
...
metrics = [3, 4, 5]  # Columns in data_points for reward, approach, screen
